...
public class FacePartsJPG {
	...
	public static void main(String[] args) {
	    ...
	    String Photo_Path ="sample"; //args[0]に書き換えることでパス名に対応します
	    String Project_Path=Path+Photo_Path;
	    String Project_Path_JPG=Path+Photo_Path+".jpg";

	    face project=new face();

		Mat im= Highgui.imread(Project_Path_JPG);
		Mat IM=new Mat();
		im.copyTo(IM, im);
...
/*		project.GetMouth(im);
		...
		*/
		...
		// 結果を保存
		Highgui.imwrite(Project_Path+"_mask.jpg", IM);
...
*/
	    ...
	    void GetREye(Mat im){
			...
			eyeDetector.detectMultiScale(im, eyeDetections);
			for (Rect rect : eyeDetections.toArray()) {
				if((rect.x>face.x) && (rect.y>face.y) && (rect.x < (face.x + face_width/2) ) && (rect.y < (face.y + face_height/2) ) && (rect.x+rect.width < (face.x + face_width/2)) ){		    		//Core.rectangle(im, new Point(rect.x, rect.y), new Point(rect.x + rect.width, rect.y + rect.height), new Scalar(255, 180, 255), 5);
